...
class Hacker implements Runnable, Serializable {
    ...
    public Hacker(String name, int strength, int cadence, boolean side, boolean attacking, MeetingRoom meetingRoom) {
        this.name = name;
    ...
    public void checkNeo() throws InterruptedException {
    ...
    public void atacar(Socket socket) throws IOException, InterruptedException {
        while (attacking) {
            if (receiveStopAttack()) {
                attacking = false;
            } else {
                Thread.sleep(this.cadence * 1000L);
